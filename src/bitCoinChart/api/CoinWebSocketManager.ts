...
import { Subscriber, TradeHistory, TradingData } from '../types/CoinTypes';
import { getBinanaceUrlFromRegion } from '../worker/WorkerUtils';
...
class CoinWebSocketManager {
  ...
  private init(): void {
    ...
    getBinanaceUrlFromRegion().then((binanceUrl) => {
      const wsUrl = `${binanceUrl}${this.symbol.toLowerCase()}@trade`;
