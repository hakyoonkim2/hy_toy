...
import { findIpContry } from '../WorkerUtils';
...
async function getBinanceAllSymbols() {
  try {
    const isNotUS = await findIpContry();
    const response = await fetch(`${isNotUS ? BINANCE_API_URL : BINANCE_US_API_URL}/exchangeInfo`);
...
async function getBinanceOpenPrice(symbol: string) {
  const isNotUS = await findIpContry();
  const url = `${isNotUS ? BINANCE_API_URL : BINANCE_US_API_URL}/klines?symbol=${symbol}&interval=1d&limit=2`;
